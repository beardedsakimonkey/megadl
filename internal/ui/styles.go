...
func renderShortcuts(shortcuts ...shortcut) string {
	...
	for _, shortcut := range shortcuts {
		...
		for _, key := range shortcut.keys {
			keys = append(keys, renderShortcutKey(key))
		}
...
// the keys themselves. A lone slash is a key (the filter shortcut), while a
// slash within a hint is only punctuation and should not inherit bold weight.
func renderShortcutKey(key string) string {
	if key == "/" {
		return styleHelpKey.Render(key)
	}
