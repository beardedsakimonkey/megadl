...
func TestFileMarker(t *testing.T) {
	tests := []struct {
		...
		{name: "queued says so rather than showing progress",
			...
			wantText: queuedGlyph, want: styleQueued.Render(queuedGlyph)},
...
func TestFilesViewRendersDirectoryTree(t *testing.T) {
	...
	// past the pane divider and the two-cell cursor column, which carries the
	// bar on the cursor row (S01E01 here) — a hairline, this pane being the
	// blurred one
